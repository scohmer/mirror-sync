...
run_container_test() {
    ...
    if command -v podman >/dev/null 2>&1; then
        echo -e "${GREEN}✓${NC} podman available: $(podman --version 2>/dev/null || echo 'version unknown')"
        
        if podman info >/dev/null 2>&1 || true; then
        ...
        echo -e "${GREEN}✓${NC} docker available: $(docker --version 2>/dev/null || echo 'version unknown')"
...
run_lock_test() {
    ...
    if lock_or_exit "$test_lock" "debug-test" 2>/dev/null || true; then
...
run_disk_test() {
    ...
    for path in "${test_paths[@]}"; do
        ...
        if check_disk_space "$path" 2>/dev/null || true; then
...
run_integration_test() {
    ...
    local runtime
    runtime="$(get_container_runtime 2>/dev/null || echo 'unknown')"
...
run_system_status() {
    ...
    if command -v podman >/dev/null 2>&1; then
        ...
        echo "  Running containers: $(podman ps -q 2>/dev/null | wc -l 2>/dev/null || echo 'unknown')"
    elif command -v docker >/dev/null 2>&1; then
        echo "  Docker: $(docker --version 2>/dev/null || echo 'version unknown')"
        echo "  Running containers: $(docker ps -q 2>/dev/null | wc -l 2>/dev/null || echo 'unknown')"
    else
...
run_container_status() {
    ...
    if ! "$runtime" ps 2>/dev/null || true; then
        echo "  No running containers or command failed"
    fi
    echo
    
    echo "All containers:"
    if ! "$runtime" ps -a 2>/dev/null || true; then
        echo "  No containers found or command failed"
    fi
    echo
    
    echo "Images:"
    "$runtime" images 2>/dev/null | head -10 || echo "  Could not list images"
    return 0
}
    ...
    if [[ -d "$base_dir" ]]; then
        ...
        for subdir in debian ubuntu rocky; do
            ...
            if [[ -d "$mirror_dir" ]]; then
                local size=$(du -sh "$mirror_dir" 2>/dev/null | cut -f1 || echo 'unknown')
                local files=$(find "$mirror_dir" -type f 2>/dev/null | wc -l 2>/dev/null || echo 'unknown')
                ...
                echo "    Files: $files"
                if [[ -n "$latest" ]]; then
                    echo "    Latest file: $(basename "$latest" 2>/dev/null || echo 'unknown')"
                    echo "    Last modified: $(stat -c %y "$latest" 2>/dev/null || echo 'unknown')"
                fi
            else
        ...
        for service in debian ubuntu rocky; do
            ...
            if [[ -d "$service_log_dir" ]]; then
                ...
                for log_type in build run; do
                    ...
                    if [[ -f "$log_file" ]]; then
                        local size=$(du -sh "$log_file" 2>/dev/null | cut -f1 || echo 'unknown')
                        local lines=$(wc -l < "$log_file" 2>/dev/null || echo 'unknown')
                        local modified=$(stat -c %y "$log_file" 2>/dev/null || echo 'unknown')
